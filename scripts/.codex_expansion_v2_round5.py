...
ROOT=Path(r"D:\python_projects\tsinghua_ai");OUT=ROOT/"data_second/public_expansion_v2";sys.path.insert(0,str(ROOT/"data_first"));from crawler.parser import detect_page,parse_html
TRACK={"utm_source","utm_medium","utm_campaign","utm_term","utm_content","spm","from"}
LISTS=[
 ("校园机构与部门","https://www.tsinghua.edu.cn/xxgk/zzjg.htm"),("清华基本信息","https://www.tsinghua.edu.cn/xxgk.htm"),("清华基本信息","https://www.tsinghua.edu.cn/xxgk/xxjj.htm"),("清华基本信息","https://www.tsinghua.edu.cn/xxgk/xrld.htm"),
 ("教学与培养","https://www.tsinghua.edu.cn/jyjx.htm"),("教学与培养","https://www.tsinghua.edu.cn/jyjx/bksjy.htm"),("教学与培养","https://www.tsinghua.edu.cn/jyjx/yjsjy.htm"),
 ("科研参与与资源导航","https://www.tsinghua.edu.cn/kxyj.htm"),("科研参与与资源导航","https://www.tsinghua.edu.cn/kxyj/kyjg.htm"),("科研参与与资源导航","https://www.tsinghua.edu.cn/kxyj/kyhz.htm"),
 ("校园文化与历史","https://www.tsinghua.edu.cn/zjqh.htm"),("校园文化与历史","https://www.tsinghua.edu.cn/zjqh/lsyg.htm"),("校园文化与历史","https://www.tsinghua.edu.cn/zjqh/xyfg.htm"),("校园综合服务","https://www.tsinghua.edu.cn/zjqh/syxx/fwxx.htm"),
 ("校园机构与部门","https://www.tsinghua.edu.cn/xxgk/yxsz.htm"),("校园机构与部门","https://www.tsinghua.edu.cn/xxgk/gljg.htm"),("校园机构与部门","https://www.tsinghua.edu.cn/xxgk/zsdw.htm"),
]
ALLOW={
"校园机构与部门":r"院|系|部|处|中心|研究院|实验室|基地|机构|委员会|书院|医院|馆|学校",
"清华基本信息":r"简介|概况|领导|校长|党委|数据|沿革|章程|校训|校徽|校歌|历史",
"教学与培养":r"本科|研究生|教学|培养|课程|项目|通识|实践|国际化|学位|教育",
"科研参与与资源导航":r"科研|研究|实验室|平台|基地|中心|仪器|资源|合作|基金|项目|学术",
"校园文化与历史":r"历史|校史|文化|传统|校园|建筑|校训|校歌|校徽|风物|艺术|博物馆|档案",
"校园综合服务":r"服务|交通|住宿|餐饮|医疗|体育|参观|校园|电话|信息",
}
...
for i,(cat,u)in enumerate(LISTS,1):
 p=OUT/"raw"/f"R5LIST{i:03d}.html";m=fetch(u,p);log.append({"list_id":f"R5LIST{i:03d}","category":cat,"url":u,**m})
 ...
 if v and v not in seen_urls:disc.setdefault(v,{"url":v,"title":title[:300],"discovery_category":cat,"discovery_source":"round5_official_p2_landing","parent_list_url":""})
 for a in s.find_all("a",href=True):
  ...
  # Cross-subdomain links are permitted only because the confirmed tsinghua.edu.cn list explicitly links them.
  disc.setdefault(v,{"url":v,"title":t[:300],"discovery_category":cat,"discovery_source":"round5_official_directory_one_level_follow","parent_list_url":m["final_url"]})
(OUT/"crawl/round5_list_log.jsonl").write_text("".join(json.dumps(x,ensure_ascii=False)+"\n"for x in log),encoding="utf-8")
sel=[];td=0
for v,x in disc.items():
 ...
 sel.append(x)
sel=sorted(sel,key=lambda x:(x["discovery_category"],x["url"]))[:650];mx=max([int(re.search(r"(\d+)",x.get("id","")).group(1))for x in existing if re.search(r"(\d+)",x.get("id",""))]or[300])
for i,x in enumerate(sel,mx+1):x["id"]=f"PUBV2-{i:04d}"
(OUT/"crawl/round5_discovered_urls.jsonl").write_text("".join(json.dumps(x,ensure_ascii=False)+"\n"for x in sel),encoding="utf-8")
got=[]
with ThreadPoolExecutor(max_workers=5)as ex:
 fut={ex.submit(fetch,x["url"],OUT/"raw"/f"{x['id']}.html"):x for x in sel}
 for j,f in enumerate(as_completed(fut),1):got.append({**fut[f],**f.result(),"crawl_timestamp":datetime.now().isoformat(timespec="seconds")});print(f"round5 fetch {j}/{len(sel)}",flush=True)if j%50==0 else None
...
(OUT/"quality_gate/round5_quality_gate_results.jsonl").write_text("".join(json.dumps(x,ensure_ascii=False)+"\n"for x in qrows),encoding="utf-8");eligible=[x for x in qrows if x.get("quality_gate_pass")];(OUT/"quality_gate/round5_audit_candidates.jsonl").write_text("".join(json.dumps(x,ensure_ascii=False)+"\n"for x in eligible),encoding="utf-8")
s={"list_pages_requested":len(LISTS),"list_pages_ok":sum(x["ok"]for x in log),"discovered":len(sel),"title_dedup":td,"fetch_ok":sum(x["ok"]for x in got),"quality_gate_pass":len(eligible),"quality_classes":dict(Counter(x.get("quality_class")for x in qrows)),"internal_dedup":internal,"by_category":dict(Counter(x["discovery_category"]for x in eligible)),"by_domain":dict(Counter(x["domain"]for x in eligible))};(OUT/"crawl/round5_summary.json").write_text(json.dumps(s,ensure_ascii=False,indent=2),encoding="utf-8");print(json.dumps(s,ensure_ascii=False))
