"""Capture a deterministic hash inventory of frozen Citation Support V1 inputs."""
    ...
    for item in SCOPES:
        ...
        if path.is_file():
            ...
            files.extend(
                ...
                if candidate.is_file() and within_project(candidate)
            )
...
def main() -> None:
    ...
    artifact = {
        "artifact": "Citation Support Runtime V1 upstream input snapshot",
    ...
    print(
        json.dumps(
            {
                "output": destination.resolve().relative_to(ROOT.resolve()).as_posix(),
                "file_count": len(rows),
