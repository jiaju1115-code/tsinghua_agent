...
PROMPT = (TEST / "prompt" / "prompt_v3_1.md").read_text(encoding="utf-8")
...
def review(item: dict):
    ...
    try:
        if raw_path.exists():
            ...
            response = client.chat({"id": ident, "access_level": "public", "source_mode": "prompt_v3_1_test"}, [{"role": "system", "content": PROMPT}, {"role": "user", "content": user}], int(CONFIG["max_completion_tokens"]), float(CONFIG["temperature"]), True)
            raw = response["choices"][0]["message"]["content"]
            obj = validate(json.loads(raw))
            raw_path.write_text(raw, encoding="utf-8")
        with lock: stats["network_request_attempts"] += session.request_attempts
        row = {**item, **obj, "model": PROVIDER.model, "reviewed_at": stamp, "prompt_version": "v3.1"}
...
with ThreadPoolExecutor(max_workers=3) as pool:
    ...
    for i, future in enumerate(as_completed(futures), 1):
        ...
        if i % 5 == 0 or i == len(INPUTS):
            print(f"[Prompt V3.1] {i}/{len(INPUTS)} 成功={len(results)} 失败={len(failures)}", flush=True)
results.sort(key=lambda r: r["id"]); failures.sort(key=lambda r: r["id"])
with (AUDIT / "v3_1_results.jsonl").open("w", encoding="utf-8") as f:
    for r in results: f.write(json.dumps(r, ensure_ascii=False) + "\n")
with (AUDIT / "v3_1_results.csv").open("w", encoding="utf-8-sig", newline="") as f:
    w = csv.DictWriter(f, fieldnames=FIELDS); w.writeheader(); w.writerows(results)
with (AUDIT / "v3_1_failures.csv").open("w", encoding="utf-8-sig", newline="") as f:
    w = csv.DictWriter(f, fieldnames=["id", "title", "error"]); w.writeheader(); w.writerows(failures)
summary = {"total_calls": len(INPUTS), "successes": len(results), "failures": len(failures), "network_request_attempts": stats["network_request_attempts"], "resumed": stats["resumed"], "actual_retries": max(0, stats["network_request_attempts"] - len(INPUTS)), "model": PROVIDER.model, "temperature": CONFIG["temperature"], "max_concurrency": 3, "max_completion_tokens": CONFIG["max_completion_tokens"], "prompt_version": "v3.1", "input_sha256": "60C385C5DA026DF0F25B03034D63DA97F62FBDF2FF365A8DD83A27E14990BC1C", "actions": dict(Counter(r["action"] for r in results)), "reject_types": dict(Counter(r["reject_type"] for r in results if r["action"] == "reject"))}
(AUDIT / "v3_1_api_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
print(json.dumps(summary, ensure_ascii=False))
if failures: raise SystemExit(f"Prompt V3.1 failures: {len(failures)}")
