...
class TrustedCampusAgentV2:
    ...
    def ask(
        ...
        cache_key = json.dumps([effective_query, as_of.isoformat()], ensure_ascii=False)
        ...
        plan = self.planner.plan(effective_query)
    ...
    def handle(
        ...
        *,
        ...
        if route.route == "rag_qa":
            result = self.ask(query, context=context)
        ...
        if needs_official_evidence:
            preliminary = self.ask(query, context=context)
