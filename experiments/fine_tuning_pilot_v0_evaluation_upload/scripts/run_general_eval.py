...
def run(label,model,tok,cases,cfg,resume):
 out=ROOT/'results'/label/'general_per_case.jsonl';out.parent.mkdir(parents=True,exist_ok=True);done={json.loads(x)['case_id'] for x in out.read_text(encoding='utf-8').splitlines() if x.strip()} if resume and out.exists() else set();fh=out.open('a' if done else 'w',encoding='utf-8')
 with torch.inference_mode():
  for c in cases:
   if c['case_id'] in done:continue
   messages=[{'role':'user','content':c['prompt']}]; inp=tok.apply_chat_template(messages,tokenize=True,add_generation_prompt=True,return_tensors='pt').to('cuda');t=time.perf_counter(); gen=model.generate(inp,max_new_tokens=cfg['max_new_tokens'],do_sample=False,pad_token_id=tok.eos_token_id);lat=(time.perf_counter()-t)*1000;raw=tok.decode(gen[0][inp.shape[1]:],skip_special_tokens=True);parsed,score,reason=score_output(raw,c);fh.write(json.dumps({'case_id':c['case_id'],'family':c['family'],'prompt_hash':hashlib.sha256(c['prompt'].encode()).hexdigest(),'raw_output':raw,'parsed_output':parsed,'score':score,'scorer_type':c['scoring_rubric']['type'],'reason':reason,'latency_ms':lat,'output_token_count':int(gen.shape[1]-inp.shape[1])},ensure_ascii=False)+'\n');fh.flush()
 fh.close()
def main():
