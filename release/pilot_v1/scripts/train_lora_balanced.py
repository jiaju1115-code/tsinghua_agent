...
def main():
    ...
    use_family_sampler=bool(cfg.get("sampling",{}).get("family_aware",False))
    sampler=(EpochAwareFamilySampler(
        families,
        seed=cfg["seed"],
        mathematical_reasoning_ratio=cfg["sampling"].get("mathematical_reasoning_target_ratio",0.35),
        num_samples=cfg["sampling"].get("epoch_sample_count",len(train)),
    ) if use_family_sampler else None)
    class Collator:
    ...
    args=TrainingArguments(output_dir=str(out),num_train_epochs=cfg["epochs"],learning_rate=cfg["learning_rate"],per_device_train_batch_size=cfg["per_device_train_batch_size"],per_device_eval_batch_size=cfg["per_device_train_batch_size"],gradient_accumulation_steps=cfg["gradient_accumulation_steps"],eval_strategy="steps",save_strategy="steps",logging_steps=cfg["logging_steps"],save_steps=cfg["save_steps"],eval_steps=cfg["eval_steps"],save_total_limit=cfg["save_total_limit"],gradient_checkpointing=cfg["gradient_checkpointing"],bf16=cfg["bf16"],fp16=cfg["fp16"],seed=cfg["seed"],report_to="none",remove_unused_columns=False)
    trainer=FamilyTrainer(model=model,args=args,train_dataset=train,eval_dataset=val,data_collator=Collator(),callbacks=([SamplingCallback()] if sampler is not None else [])); trainer.train(resume_from_checkpoint=a.resume_from_checkpoint); metrics=trainer.evaluate(); model.save_pretrained(out); tok.save_pretrained(out)
